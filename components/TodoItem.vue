<template>
  <div data-cy="todo-item" class="flex items-center justify-between p-5 bg-white shadow-sm rounded-md mb-4">
    <div v-if="editing" data-cy="editing-mode" class="flex-grow">
      <input v-model="newTitle" data-cy="new-text" type="text" class="mr-5 p-2 bg-gray-200 rounded-md" />
      <input v-model="newDueDate" data-cy="new-date" type="date" class="p-2 bg-gray-200 rounded-md" />
    </div>
    <div v-else class="flex-grow">
      <input type="checkbox" :checked="todo.completed" class="mr-5" @click="toggleTodo" />
      <span :class="{'line-through': todo.completed, 'bg-red-200': isPastDue}">
